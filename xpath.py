...
def all_nodes_extracted_check(tree, depth):
    ...
    while index < len(queue) and output:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            threads = [
                queue[thread] for thread in range(index, min(index + 5, len(queue)))
            ]
...
def gen_xml():
    ...
    while not nodes_done:
        ...
        for idx, i in enumerate(tree[depth]):
            ...
            while not done:
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    threads = [
                        ("node", tree[depth][idx][0], thread)
                        for thread in range(index, index + 5)
                    ]
                    for result in executor.map(map_helper, threads):
                        if result == "":
                            ...
                            node = f"{node}[{count}]"
                            tree[depth + 1].append(
                                (node, ET.SubElement(tree[depth][idx][1], result))
                            )
                index += 5
    ...
    while index < len(queue):
        with concurrent.futures.ThreadPoolExecutor() as executor:
            threads = [
                ("content", tree[queue[i][0]][queue[i][1]][0], 1)
                for i in range(index, min(index + 5, len(queue)))
            ]
            for idx, result in enumerate(
                executor.map(map_helper, threads), start=index
            ):
                print(
                    tree[queue[idx][0]][queue[idx][1]][0].split("/")[-1].capitalize()
                    + ": "
                    + result
                )
